...
- [Conceptos básicos](#conceptos-básicos)
  ...
  - [Usar Vue](#usar-vue)
  - [Aplicación Vue básica](#aplicación-vue-básica)
    - [HTML](#html)
...
Este material está basado en la [guía oficial de Vue](https://vuejs.org/guide/introduction.html) y veremos además los servicios de vue-router, axios y pinia entre otros.
...
* **Rendimiento**: Vue hace uso del concepto de _Virtual DOM_ similar a React por lo que también ofrece muy buen rendimiento
...
Sin embargo esta no es la forma más recomendable de trabajar. Lo normal es crear un proyecto con **_npm_** que genere un completo _scaffolding_. Esto nos permitirá trabajar con componentes (_Single File Components_ o _SFC_) lo que nos facilitará enormemente la creación de nuestras aplicaciones.

Un _SFC_ es un componente reutilizable que se guarda en un fichero con extensión _.vue_. Para que _VSCode_ reconozca correctamente los ficheros _.vue_ debemos instalar la _extensión_ **Vue-Official**.

## Aplicación Vue básica
Vamos a ver un ejemplo muy sencillo de una aplicación Vue creada sin usar ninguna herramienta adicional, simplemente enlazando la librería desde un CDN. Mostrará un contador y un botón para actualizarlo:
...
### Javascript
En el fichero JS debemos crear la aplicación con el método _createApp_ y montarla en el elemento del HTML donde se ejecutará dicha aplicación. Al método _createApp_ se le pasa un objeto con una serie de propiedades, en nuestro caso **data** y **methods**, pero hay muchas más:
- **`data`**: aquí es donde se define el _estado_ de la aplicación, es decir, los datos de la misma. Es una **función** que _devuelve un objeto_ donde cada propiedad es una variable a declarar. Lo que definimos aquí sería el equivalente a las propiedades definidas en una clase que almacenan el estado de la misma. Estos datos son reactivos y accesibles desde el HTML
...
Fijaos que para hacer referencia desde Javascript a una variable (o a un método) hay que anteponerle **_this_** igual que en una clase.

Además de las opciones _data_ y _methods_ podemos definir otras como:
* **computed**: es un objeto con funciones que devuelven una variable con un valor calculado. Por ejemplo:
```javascript
data: () => ({
  products: [
    {name: 'Bolígrafo', price: 1.5, units: 26},
    {name: 'Cuaderno', price: 3.0, units: 12}
  ],
}),
computed: {
  totalImport() {
...
Anteriormente había que instalarla pero ahora viene integrada en la herramienta de desarrollador de _Vite_.
