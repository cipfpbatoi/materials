...
Para usar TS en un componente tenemos que indicarlo en la etiqueta `<script>`:
```vue
<script lang="ts">
  // código Typescript
</script>
...
## Tipos de datos
Los tipos de datos que podemos encontrar en Typescript son:
- String
- Number
- Boolean
- Array
...
También permite que una variable pueda tener más de un tipo de datos utilizando el operador `|` (OR):
```typescript
...
```typescript
type Module = 'DWEC' | 'DWES' |'DIW' |'DAW' | 'EIE' | 'Inglés'

interface Student {
  name: string;
  age: number;
  modules: Module[];   // o también modules: Array<Module>
}
...
```typescript
function addItemToList<Type>(item: Type, list: Type[]): Type[] {
    list.push(item)
