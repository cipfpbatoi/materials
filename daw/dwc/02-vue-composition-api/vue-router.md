...
- [Vue-router](#vue-router)
  ...
  - [Aplicación de ejemplo](#aplicación-de-ejemplo)
    ...
    - [App.vue](#appvue)
    - [components/TodoItem.vue](#componentstodoitemvue)
    - [views/ViewItem.vue](#viewsviewitemvue)
...
En resumen, en nuestra aplicación (normalmente en el _App.vue_) tendremos una etiqueta `<router-view>` y lo que hará _vue-router_ es cargar en esa etiqueta el componente que corresponda en función de la ruta que haya en la barra de direcciones del navegador. Por ejemplo si la URL es **/products** podría cargar un componente llamado _ProductsTable_ (que mostrará una tabla con todos los productos de la aplicación) y si la URL es **/newprod** podría cargar un componente llamado _ProductForm_ con un formulario para añadir un nuevo producto.
...
- se importa dicho fichero en el **main.js** para que el almacén esté disponible para todos los componentes:
...
Y ya la tenemos disponible en la variable `router`.
...
- **`.go(num)`**: permite saltar el num. de páginas indicadas adelante (ej. _router.go(1)_) o atrás (_router.go(-1)_) por el historial
...
salta a la URL `/register?plan=private`. En el componente que se carga obtenemos la query pasada con `route.query` (obtenemos un objeto, en el ejemplo `{ plan: 'private' }`).
...
Y ya la tenemos disponible en la variable `route`.
...
Podés obtener más información en la [documentación de Vue-router](https://router.vuejs.org/guide/essentials/redirect-and-alias.html).
...
O en la _Options API_ usaríamos la propiedad _watch_ del componente:
```javascript
export default {
  watch: {
      '$route' (to, from) {
...
Cada vez que cambie el valor de _route_ se ejecutará ese código y recibirá en el parámetro _to_ la nueva ruta y en _from_ el valor anterior de la variable. Veremos los _watchers_ más adelante o podéi consultar la [documentación de Vue](https://vuejs.org/guide/essentials/watchers.html).
...
Vamos a hacer que nuestra aplicación de la lista de tareas use _vue-router_ para tener varias vistas: una con la lista de tareas, otra con un formulario para añadir nuevas tareas, otra con información sobre cada tarea (añadiremos en _TodoItem.vue_ un botón de 'Ver') y una con información acerca de nosotros. Además crearemos un menú para navegar entre las diferentes vistas.

Podéis ver el código completo de la aplicación en este [repositorio de GitHub](https://github.com/juanseguravasco/vue-todo-list/tree/6-vue-router).
...
Definimos las rutas para las 4 vistas que tendrá nuestra aplicación. La ruta dinámica _/view/:id_ cargará el componente _ViewItem_  en cualquier ruta que comience por _/view/_ y le pasará a dicho componente un parámetro llamado _id_.
...
Ya no hay sección de _script_ porque no necesitamos código en este componente. En el _template_ hemos creado un menú con enlaces a las diferentes rutas y hemos puesto la etiqueta `<router-view>` donde se cargarán los componentes correspondientes a cada ruta.
...
Obtiene el parámetro _id_ de la ruta usando `route.params.id` y usa ese valor para obtener la tarea del almacén y mostrar sus datos, como vemos en las _Dev Tools_:

![Vue Dev Tools mostrando la vista de una tarea](./img/devTools_TodoItem.png)

En el _store_ hemos añadido el método _getTodoById(id)_ que devuelve la tarea con el id indicado:
```javascript
  getTodoById(id) {
