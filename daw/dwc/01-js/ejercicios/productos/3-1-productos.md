...
Esta clase tendrá los siguientes **métodos**:
  - **changeUnits**: recibe el número de unidades a añadir al objeto y devuelve el producto modificado. Si se intentan restar más unidades de las que hay no resta nada y genera un error.
  ```javascript
  ...
  ```  
  - **xxxx** (¿qué nombre le deberías dar a este método?): si se intenta **imprimir** el producto (por ejemplo concatenándolo a una cadena) se mostrará su descripción, sus unidades entre paréntesis, su precio y el importe total (los € siempre con 2 decimales) como en el siguiente ejemplo:
```
...
La clase tendrá los **métodos**:
  ...
  ```
  - **changeProductUnits**: recibe como **único** parámetro **un objeto** con la id de un producto y el número de unidades a incrementar o disminuir (si el número es negativo). Devuelve el producto modificado si ha podido hacerse o generará un error en caso contrario o si:
  - no se le pasa _id_ o no existe el producto en el almacén
  - no se le pasa _units_ o no es un número entero
  ```javascrpt
  changeProductUnits(id: Integer, units: Integer): Product
  ```  
  - **changeProduct**: recibe como **único** parámetro **un objeto** con la id de un producto y uno o más campos a modificar con sus nuevos valores (los campos que no se pasan permanecerán sin cambios). Devuelve el objeto modificado si ha podido hacerse o genera un error en caso contrario o si:
  - no se le pasa _id_ o no existe el producto en el almacén
  - se le pasa _price_ y no es un número positivo
  - se le pasa _units_ y no es un número entero positivo
  ```javascrpt
...
- **index.js**: el programa principal que crea el almacén. Además para probar que funciona lo modificará (añadiendo, eliminando y modificando productos) y mostrará por consola su contenido

En el _index.html_ habría que enlazar los 3 ficheros en el orden correcto (productos, almacén y por último index para que desde _index.js_ se pueda llamar a métodos de _Store_ y desde _store.js_ a métodos de _Product_). Como esto ya empieza a ser incómodo vamos a hacer uso de _webpack_ para que empaquete todos nuestros ficheros javascript en un único fichero que se llamará _./dist/main.js_ y que será el único que enlazaremos en el _index.html_. Consulta [cómo usar webpack](../12-tests.html#usar-webpack) para hacerlo. 
