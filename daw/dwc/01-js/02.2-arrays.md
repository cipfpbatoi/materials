...
```
También podemos pasarle una función que le indique cómo ordenar, que devolverá un valor negativo si el primer elemento es mayor (no hace nada), positivo si es mayor el segundo (los invierte) o 0 si son iguales. Ejemplo: ordenar un array de cadenas sin tener en cuenta si son mayúsculas o minúsculas:
```javascript
...
let alumnosOrdenado = alumnos.sort(function(alumno1, alumno2) {
  return alumno1.age - alumno2.age  // orden ascendente
})
```
