...
- _view_: aquí crearemos un fichero JS que será el encargado de la UI de nuestra aplicación. Tendremos una única clase que representa toda la vista aunque en aplicaciones mayores lo normal es tener clases para cada página, etc
...
La vista será una clase cuyas propiedades serán nodos de la página que vayamos a modificar, para no tener que buscarlos cada vez y para que estén disponibles para el controlador. Contendrá métodos para renderizar los distintos elementos de la vista.
...
class StoreView{
    constructor {
        ...
        this.productsList = document.getElementById('productsTable')
    }
