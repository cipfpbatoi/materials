...
Si nuestra aplicación no va a ser muy pequeña como la del ejemplo sino que tendrá diferentes ficheros Javascript, acceso a datos, interfaz de usuario, etc deberíamos organizar en código en carpetas para mejorar su legibilidad. Lo adecuando es crear una carpeta `src` donde poner todos los ficheros `.js` que es donde estará el código. En función del tamaño de la aplicación podríamos crear dentro subcarpetas (os propongo un nombre para ellas aunque podéis llamarlas como queráis) para:
...
El proyecto creado contiene varios ficheros y carpetas importantes:
- `index.html`: es el fichero HTML principal de la aplicación. Contiene una etiqueta `<script type="module" src="/src/main.js"></script>` que carga el módulo _main.js
- `src/main.js`: es el punto de entrada a la aplicación. Importa los ficheros CSS, imágenes y ficheros JS con funciones o clases y establece el contenido de la página principal
- `src/counter.js`: módulo JS que exporta una función como ejemplo que es usada en el _main.js_
- `src/style.css`: fichero donde poner nuestros estilos, con CSS de ejemplo
- `public/`: carpeta donde dejar elementos estáticos que no pasarán por _vite_ (como imágenes, ficheros CSS, ...)
- `node_modules`: librerías de las dependencias (si usamos alguna). Se creará al instalar las dependencias con `npm install`
- `package.json`: fichero de configuración del proyecto. Además del nombre y la versión
