...
-   **Debian**: es uno de los sistemas más utilizados por ser de los más eficientes en la gestión de los recursos del sistema. En caso de decantarnos por esta distribución deberemos escoger qué versión instalar. Para un servidor la opción recomendada es la versión estable (en la actualidad la versión 10 llamada **Buster**) aunque, en función de nuestras necesidades,  también podríamos decantarnos por la versión testing (actualmente la 11 llamada **Bullseye**).
-   **Ubuntu Server**: también es muy utilizada y se trata de una distribución específica para servidores (o sea, que en la imagen de la misma se incluyen paquetes que suelen usarse en servidores en vez de paquetes de clientes). En este caso es más que recomendable instalar la última versión **LTS** que tiene soporte para 5 años ya que en caso de instalar una versión "normal" deberemos estar actualizando la versión de nuestro servidor cada 6 meses. En la actualidad es la 18.04 (**Bionic Beaver**) aunque en próximo mes de abril saldrá la nueva versión LTS 20.04.
-   **Cent OS**: es un fork gratuito de RHEL y desde 2014 cuenta con el apoyo de Red Hat. Se trata de una distribución muy utilizada al estar basada en la Red Hat Enterprise Linux que es muy robusta. La versión actual es la 8
...
Como vimos en Windows una de las tareas más importantes a la hora de planificar la instalación del servidor es decidir las particiones que haremos y qué sistema de archivos usaremos en cada una.

En GNU/Linux siempre usaremos una partición para swap pero, además de ella, podemos usar una única partición para datos y sistema (no se recomienda) o usar varias particiones. En este caso algunos de los directorios que podríamos montar en particiones diferentes son:
...
Lo más común es instalar el servidor sin interfaz gráfica. Si queremos administrarlo gráficamente por comodidad podemos instalar alguna herramienta como **Webmin** y administrar el servidor desde cualquier máquina con entorno gráfico y un navegador.

### Instalación de Debian 8 Jessie

[installDebian.ogv](installDebian.ogv)
...
![CentOS](00-centos.png "CentOS")

Lo primero que nos pregunta es el idioma:

![CentOS](01idioma.png "CentOS")
...
![Repositorios](repDebian.jpg "Repositorios")![Instalar software](tasksel.jpg "Instalar software")
...
Webmin es una herramienta web que permite configurar remotamente equipos GNU/Linux desde otro ordenador que disponga de entorno gráfico y navegador web.

Permite configurar la red, usuarios, cuotas de disco, servicios, archivos de configuración, apagado del equipo, etcétera, así como modificar y controlar muchas aplicaciones como el servidor web Apache, PHP, MySQL, DNS, Samba, DHCP, LDAP, etc.

### Instalar Webmin

En la web de Webmin tenemos amplia información sobre cómo descargar e instalar Webmin en diferentes sistemas operativos. Aquí veremos cómo hacerlo en Debian/Ubuntu y el CentOS.

La instalación de Webmin nos plantea un problema ya que no está en los repositorios por lo que no se puede instalar directamente.

Para instalar un paquete que no se encuentra en los repositorios tenemos 2 opciones:

-   descargarlo e instalarlo nosotros mismos, tal y como se hace en sistemas Windows
-   añadir a nuestros repositorios el repositorio donde se encuentra dicho paquete

#### Descarga manual

Vamos a ver cómo descargarlo manualmente. Para ver cómo añadirlo a los repositorios consultad la página de Webmin en:

[http://webmin.com/index.html](http://webmin.com/index.html)

En primer lugar hemos de localizar el URL desde donde descargarlo (para ello desde un equipo con entorno gráfico buscamos el programa en la web de webmin) y lo descargamos en el servidor con el comando wget. En Debian/Ubuntu haríamos:

  wget http://prdownloads.sourceforge.net/webadmin/webmin-1.930\_all.deb

En CentOS (el paquete a descargar es el empaquetado en formato rpm:

  wget http://prdownloads.sourceforge.net/webadmin/webmin-1.930-1.noarch.rpm

Una vez descargado nuestro paquete lo instalamos. En Debian/Ubuntu:

  dpkg -i webmin-1.930\_all.deb

En CentOS:

  yum -U webmin-1.930-1.noarch.rpm

Si faltara alguna dependencia deberemos descargarla e instalarla (recordad apt-get -f install).

#### Configuraciones previas

Ya tenemos nuestro webmin instalado. Para arrancar el servicio haremos:

  /etc/init.d/webmin start

En CentOS, si queremos que arranque automáticamente al iniciar el sistema haremos:

  chkconfig webmin on

Por último falta abrir el firewall de CentOS para que permita conexiones entrantes por el puerto de webmin, el 10000:
...
Y ya podemos abrir desde otro equipo el webmin poniendo en el navegador la dirección:

***https://IP\_o\_nombre\_del\_servidor:10000***

Documentación de la instalación
===============================

Como vimos al hablar de los servidores en Windows es imprescindible documentar nuestros equipos, especialmente los servidores. Esta documentación no es algo estático sino que se irá ampliando y modificando según vamos cambiando la configuración de la máquina.

Podemos utilitzar el formato que más nos guste o usar alguna plantilla de internet. Podéis ver un ejemplo en [https://moodle19.cipfpbatoi.es/pluginfile.php/30300/mod\_resource/content/1/documentaci%C3%B3\_equip.pdf](https://moodle19.cipfpbatoi.es/pluginfile.php/30300/mod_resource/content/1/documentaci%C3%B3_equip.pdf).

En la Unidad 7 se indica toda la información que deberíamos incluir en la documentación del servidor.
...
Vamos a continuar con nuestro proyecto del aula para la ESO, pero esta vez instalaremos y configuraremos un servidor con software libre.

Ya conocemos los requerimientos del sistema informático a implementar y ya decidimos en la práctica de la unidad 7 que para ello necesitamos montar una red cliente/servidor. Ahora vamos a instalar y configurar el servidor del aula.

Como ya sabemos las tareas que deberemos realizar son:

-   elegir que sistema operativo tendrá nuestro servidor. Como hemos dicho escogeremos un sistema operativo libre
-   realizar la planificación de la instalación
-   instalar el sistema operativo
-   configurarlo correctamente
-   configurar los discos y el enrutamiento
-   instalar Webmin para configurar el servidor más fácilmente
...
Hemos de localizar el URL desde donde descargarlo (para ello desde un equipo con entorno gráfico buscamos el programa en la web de webmin) y lo descargamos en el servidor con el comando wget:

  wget http://prdownloads.sourceforge.net/webadmin/webmin-1.760-1.noarch.rpm

OJO: el paquete a descargar no es el .deb (el instalador para Debian y derivados) sino el .rpm (instalador para RedHat y derivados).

Una vez descargado nuestro paquete lo instalamos:

  yum -U webmin-1.760-1.noarch.rpm

Si faltara alguna dependencia deberemos descargarla e instalarla (con el comando yum).

Otra opción es añadir a mi lista de repositorios los repositorios oficiales de webmin. Una vez hecho esto y actualizada la lista de paquetes ya tendremos el programa webmin disponible para su instalación como cualquier otro. Para añadir el repositorio de webmin debemos crear un nuevo fichero de texto al que llamaremos **/etc/yum.repos.d/webmin.repo** con el siguiente contenido:
...
A continuación descargamos e importamos la clave de repositorio:

  wget http://www.webmin.com/jcameron-key.asc

  rpm --import jcameron-key.asc

Tras actualizar la lista de paquetes ya podemos descargar webmin como cualquier otro programa con:
...
Si queremos que arranque automáticamente al iniciar el sistema haremos:

  chkconfig webmin on

Por último falta abrir el firewall para que permita conexiones entrantes por el puerto de webmin, el 10000:
...
https://IP o nombre del servidor CentOS:10000
