...
1. **Vore la configuració de la xarxa** d'eixe equip per a comprovar que tot és correcte (IP, màscara, porta d'enllaç i DNS). Ho farem amb `networkctl status` (si és un equip Windows amb `ipconfig /all`)
...
2. **Fer un `ping` a la porta d'enllaç**. Si la configuració és correcta fem un _ping_ a la porta d'enllaç que ens ha mostrat el comando del pas anterior. Si falla pot ser:
  ...
  - _el cable_: el cable que ix del nostre equip ha d'arribar al mateix switch al que va el de la nostra porta d'enllaç. Si és una màquina virtual s'hem d'assegurar que el nom de la targeta interna del client i del servidor és el mateix (és el nom del switch virtual que es crea). Si la màquina te vàries targetes de xarxa hem d'assegurar-nos de quina és cadascuna comprovant la MAC de Virtualbox amb la del sistema operatiu.
  - _el firewall_: el firewall dels Windows clients (Windows 10, ...) NO permet el pas dels pings. Per això no farem ping al client sinó des del client al servidor. Si el servidor és Windows Server 2022 o posterior i el client no està afegit al domini el Firewall talla els pings i haurem d'afegir una regla al firewall per a permetre els paquets ICMP (o deshabilitar el firewall i tornar-ho a habilitar tras comprovar el ping)

3. **Fer un `ping` a un equip de fora de la xarxa**. Si el ping anterior ha funcionat farem un ping a un equip de fora, per exemple al 8.8.8.8 (si estem en l'institut podem fer-ho al servidor del centre, el 172.16.20.1, perquè el router no deixa eixir els pings fora). Si falla és
